<script lang="ts">
  ...
  // Reactive scroll to bottom logic
  ...
  // Ensure that the container scrolls when the component is updated
  afterUpdate(() => {
    if (autoscroll && chatContainer) {
      chatContainer.scrollTop = chatContainer.scrollHeight;
    }
...
>
  {#if loading_status}
    <StatusTracker
      ...
      {...loading_status}
      show_progress={
        loading_status.show_progress === "hidden"
          ? "hidden"
          : "minimal"
      }
    ...
    />
  {/if}
  <div class="wrapper" bind:this={chatContainer}>
    {#if show_label}
      <BlockLabel
        {show_label}
        Icon={Chat}
